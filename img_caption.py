...
class WorkerThread(QThread):
    ...
    def upload_zip_json_zh(self, image_dir, json_path, url):
        # 请求单个文件的结果
        ...
        if self.exist:
            # 获取服务器上文件路径
            jsonname = os.path.basename(json_path)
            shared_image = (
                "./shared_data/Chinese_image_caption/images/" + jsonname[0:-4]
            )
            shared_json_file = (
                "./shared_data/Chinese_image_caption/json_files/" + jsonname
            )
            data = {
                "isFileExist": self.exist,
                "json_file": shared_json_file,
                "image_folder": shared_image,
            ...
            # 创建内存中的ZIP文件
            # image_dir = ("/data1/zxy/data/lenovo/suzhou/images/blip_laion_cc_sbu_zh_230801_350k")

            zip_buffer = io.BytesIO()
            zip_directory(image_dir, zip_buffer)

            # json_path = "/data1/zxy/data/lenovo/suzhou/json_files/blip_laion_cc_sbu_zh_230801_350k.json"
            ...
            # 发送ZIP文件
            print(os.path.basename(image_dir) + ".zip")
            files = {
                "file": (
                    os.path.basename(image_dir) + ".zip",
                    zip_buffer,
                    "application/zip",
                ),
                "json_file": (
    ...
    def upload_zip_json_en(self, image_dir, json_path, url, task_type):
    ...
    def run(self):
        ...
        if self.language == "中文":
            ...
            for json_name in self.json_list:
                image_dir = (
                    os.path.dirname(self.folderpath) + "/images/" + json_name[0:-5]
                )
                json_path = self.folderpath + "/" + json_name
                print(image_dir)
                print(json_path)
                print(url)
                response = self.upload_zip_json_zh(image_dir, json_path, url)
            ...
            for json_name in self.json_list:
                ...
                print(json_path)
                print(url)
                response = self.upload_zip_json_en(image_dir, json_path, url, task_type)
...
class Tab_img_caption(TabWidget):
    def __init__(self, ui) -> None:
        super().__init__()
        # 设置组件
        self.image_label: QLabel = ui.label_3
        self.mode_comboBox: QComboBox = ui.comboBox_10  # 长短文本
        self.language_comboBox: QComboBox = ui.comboBox_12  # 语言
        self.value: QLineEdit = ui.lineEdit_3  # 阈值
        # self.model_combobox: QComboBox = ui.comboBox_3  # 模型
        self.upload_button: QPushButton = ui.pushButton_9
        self.download_button: QPushButton = ui.pushButton_27
        self.pre_butt: QPushButton = ui.pushButton_8
        self.next_butt: QPushButton = ui.pushButton_7
        self.select_filefolder_butt: QPushButton = ui.pushButton_6
        self.file_comboBox: QComboBox = ui.comboBox
        self.answerEdit: QTextEdit = ui.textEdit_3
        self.table: QTableWidget = ui.tableWidget_3
        self.update_comboBox: QComboBox = ui.comboBox_7
        self.try_butt: QPushButton = ui.pushButton_40  # 测试文件是否在服务器
        ...
        # 设置ip
        self.chinese_url = self.ip + ":5011"
        self.english_url = self.ip + ":5987"
    ...
    def on_language_selected(self):
        """切换语言和显示的模型"""
        if self.language_comboBox.currentText() == "中文":
            # Chinese
            self.model_num = 4
            self.model_list = ["qwen", "share4v", "visglm", "result"]
            # self.set_model(self.model_list, self.model_combobox)

        else:
            # English
            if self.mode_comboBox.currentText() == "短文本":
                self.model_num = 7
                self.model_list = [
                    "qwen",
                    "transcorem",
    ...
    def upload_t(self):
        ...
        self.thread = WorkerThread(
            ...
            self.folderpath,
            int(self.value.text()),
            True if self.mode_comboBox.currentText() == "短文本" else False,
    ...
    def test_file_exist(self):
        ...
        if self.language_comboBox.currentText() == "中文":
            ...
            self.file_exist = TabWidget.Is_file_exist(
                self.imgpath, self.exist_url, 1, 2
            )
        else:
            # 英文长文本
            if self.mode_comboBox.currentText() == "长文本":
                self.file_exist = TabWidget.Is_file_exist(
                    self.imgpath, self.exist_url, 1, 7
                )
            else:
                # 英文短文本
                self.file_exist = TabWidget.Is_file_exist(
                    self.imgpath, self.exist_url, 1, 6
                )
        if self.file_exist:
    ...
    def run(self):
        ...
        # 参数框
        self.mode_comboBox.addItems(["长文本", "短文本"])
        ...
        # 设置初始语言为中文
        self.language_comboBox.addItems(["中文", "英文"])
        self.language_comboBox.setCurrentIndex(0)
        self.on_language_selected()

        # 切换语言响应
        self.language_comboBox.activated.connect(self.on_language_selected)
